...
class APIAgent:
    ...
    def run(
        ...
        *,
        ...
        final_text = ""
        iteration = 0
        # 本轮内"相同工具 + 相同参数"只真正执行一次。
        # 模型有时会重复发起同一个调用（实测 get_weather 被连调两次），
        # 复用结果既省延迟与费用，也避免带副作用的工具被误执行两次。
        ...
        while iteration < self.max_iterations:
            ...
            if out["tool_calls"]:
                ...
                for i, tc in enumerate(out["tool_calls"]):
                    ...
                    cache_key = f"{name}|{raw_args.strip()}"
                    if cache_key in executed:
                        result = executed[cache_key]
                        t_log.log("action.deduplicated", tool=name, args=args_obj,
                                  note="本轮已用相同参数调用过，复用上次结果，不重复执行")
                    else:
                        result = self.registry.dispatch(name, raw_args)
                        executed[cache_key] = result
                        t_log.action_result(name, result.to_dict(), result.duration_ms,
                                            ok=result.ok)
                        if on_action:
                            on_action(record_for(result, name, args_obj, iteration, tc))

                    resp.tool_calls.append(
                        ToolCallRecord(name=name, args=args_obj, result=result,
...
def record_for(result: ToolResult, name: str, args: Any, iteration: int,
               tc: Dict[str, Any]) -> ToolCallRecord:
    """构造一条"真实执行过"的行为记录（供 on_action 回调使用）。"""
    return ToolCallRecord(name=name, args=args, result=result,
                          iteration=iteration, call_id=tc.get("id", ""))
