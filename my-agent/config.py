...
class Settings:
    ...
    # --- Lv3：Live2D 表现层 ---
    # 表现层服务端口。0 表示让系统自动分配（随机空闲端口）
    avatar_port: int = 8765
    # 服务就绪后自动打开浏览器。交给程序来做（而不是让 .bat 猜时机），
    # 这样无论端口有没有被占用、Ollama 起动慢不慢，打开的地址都一定是对的。
    avatar_auto_open: bool = True
    ...
    def load(cls) -> "Settings":
        return cls(
            ...
            avatar_port=_env_int("AGENT_AVATAR_PORT", 8765),
            avatar_auto_open=_env_bool("AGENT_AVATAR_AUTOOPEN", True),
        )
...
# Lv3：Live2D 表现层
# --------------------------------------------------------------------------
# 题目 Lv3 的关键是"Avatar 是否真正成为 Agent 状态的一种输出"。
# 所以映射表放在**后端**（这里），而不是前端 JS 里 —— 前端只负责画。
# 这样"她为什么摆这个表情"是可解释、可审计的，也能写进日志。
#
# 模型自带的 8 个表情（读自 miku/*.exp3.json，它们驱动的是开关参数）：
#   sing  唱歌 (Param134)      heart 比心 (Param135)     leek 大葱 (Param133)
#   blush 脸红 (Param130)      lean  前倾 (Param132)     swirl 圈圈 (Param125)
#   chibi QQ人 (Param131/136)  watermark 水印 (Param137，不使用)
#
# 注意：sing / heart / leek 三个是**互斥**的（同一组参数 133/134/135），
# 所以它们只能出现在 primary 里，不能同时叠加。
# 而 blush / lean / swirl / chibi 各用独立参数，可以叠加在 primary 之上。
LV3_EXPRESSION_MAP: Dict[str, Dict[str, Any]] = {
...
# 特别开心的时候掏出标志物——大葱。这是"情绪强度"而不只是"情绪种类"的体现。
LV3_HAPPY_LEEK: Dict[str, Any] = {
    "min_valence": 0.80,
    "min_arousal": 0.80,
    "expression": "leek",
}

# 动作组：这个模型只带一段 2.67 秒的循环动作（Scene1），已注册为 Idle。
# 想让不同心情播不同动作，就再往 model3.json 的 Motions 里加组名并在这里映射。
LV3_MOTION_MAP: Dict[str, str] = {mood: "Idle" for mood in LV3_EXPRESSION_MAP}

# 正在调用工具时的表现（她用"圈圈"表示自己在努力查资料）
LV3_ACTING_EXPRESSION: str = "swirl"

# 水印：模型作者在《模型使用说明.txt》里写明「水印按键默认打开，
# 需在设置表情中关闭」。所以这里默认**尊重作者设定 = 打开水印**。
# 想关掉（作者也允许）就改成 False —— 前端会把 Param137 归零。
# 注意：本播放器会主动把不活跃的表情参数归零，所以必须显式声明这个开关。
LV3_SHOW_WATERMARK: bool = True
