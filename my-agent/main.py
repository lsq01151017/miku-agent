...
# Lv3 表现层（Live2D）。用 try 包住：没有这部分也不该影响 Lv1/Lv2 运行。
try:
    from live2d.avatar_server import AvatarServer  # noqa: E402
    AVATAR_IMPORT_ERROR: Optional[str] = None
except Exception as _avatar_exc:  # pragma: no cover
    AvatarServer = None  # type: ignore[assignment]
    AVATAR_IMPORT_ERROR = f"{type(_avatar_exc).__name__}: {_avatar_exc}"
...
class Orchestrator:
    ...
    def __init__(self, session_id: Optional[str] = None, verbose: bool = True,
                 ...
                 router_mode: Optional[str] = None, avatar: bool = False,
                 avatar_port: Optional[int] = None):
        """db_path / state_path 可覆盖，用于让测试用独立的临时存储，
        绝不去污染你真实的记忆库与情绪状态。

        avatar=True 时启动 Live2D 表现层服务（Lv3），浏览器打开它给出的地址
        就能看到"她的表情跟着内部状态变"。
        """
        ...
        self.avatar_error: Optional[str] = AVATAR_IMPORT_ERROR
        if avatar:
            if AvatarServer is None:
                self.avatar_error = self.avatar_error or "AvatarServer 不可用"
            else:
                try:
                    self.avatar = AvatarServer(
                        port=avatar_port if avatar_port is not None else SETTINGS.avatar_port,
                        verbose=False,
                    )
                    self.avatar.start()
                except Exception as exc:
                    self.avatar = None
                    self.avatar_error = f"启动失败 {type(exc).__name__}: {exc}"

    # ---------------- Lv3：状态推送 ----------------
    def _avatar(self, **fields: Any) -> None:
        """推送行为状态（思考/说话/查询/字幕）。任何异常都不该影响对话。"""
        if self.avatar is None:
            return
        try:
            self.avatar.update(**{k: v for k, v in fields.items() if v is not None})
        except Exception:
            pass

    def _avatar_emotion(self, **extra: Any) -> None:
        """推送情绪快照 —— 表情映射在 AvatarServer 内部按 config 的表算。"""
        if self.avatar is None:
            return
        try:
            self.avatar.push(self.state.snapshot(),
                             name=self.memory.user_title(), **extra)
        except Exception:
            pass
    ...
    def startup(self, turn: Optional[TurnContext] = None) -> Optional[str]:
        ...
        if not self._logged_startup:
            ...
            # 表现层先摆好当前状态，页面一打开就是"对的她"
            self._avatar_emotion(thinking=False, acting=False, speaking=False,
                                 text="", route="", tools=[])
            if self.avatar is not None:
                self.log.event("avatar.ready", url=self.avatar.url,
                               expressions=sorted(self.avatar.expression_params),
                               mood=self.state.mood)
            elif self.avatar_error:
                self.log.event("avatar.unavailable", level="warn",
                               error=self.avatar_error)
    ...
    def health(self) -> str:
        ...
        if self.avatar is not None:
            lines.append(f"  Live2D 表现层 (Lv3): {paint(self.avatar.url, C_MIKU)}"
                         f"  已连接客户端 {self.avatar.client_count}")
        elif self.avatar_error:
            lines.append(f"  Live2D 表现层 (Lv3): {paint('未启用', C_WARN)}  {self.avatar_error}")
        return "\n".join(lines)
    ...
    def handle(self, user_text: str, *, stream: Optional[bool] = None,
        ...
        # ---- 2.5) 表现层：进入"思考中" ----
        self._avatar_emotion(thinking=True, acting=False, speaking=False,
                             text="", route=decision.route, tools=[],
                             turn=turn.turn)
        ...
        # 表现层：说出结论 + 落到新情绪/新表情（Lv3 的闭环）
        if self.avatar is not None:
            self._avatar_emotion(thinking=False, acting=False, speaking=False,
                                 text=result.reply, route=result.route,
                                 tools=[t.get("tool") for t in result.tools],
                                 turn=turn.turn)
            self.log.event("avatar.expression", mood=self.state.mood,
                           valence=round(self.state.values["valence"], 3),
                           arousal=round(self.state.values["arousal"], 3),
                           route=result.route, clients=self.avatar.client_count)
        return result
    ...
    def _handle_local(self, user_text: str, decision: RouteDecision, turn: TurnContext,
        ...
        def on_token(piece: str) -> None:
            ...
            # 表现层：一开始吐字就让她"开口"（口型由前端按 speaking 驱动）
            if not final_text[:-1]:
                self._avatar_emotion(thinking=False, speaking=True)
    ...
    def _handle_api(self, user_text: str, decision: RouteDecision, turn: TurnContext,
        ...
        def on_action(record) -> None:
            ...
            if self.verbose:
                ...
                print(paint(f"\n  ⚙ 未来正在查… {record.name}"
                      ...
                      flush=True)
            # 表现层：正在执行工具 —— 她会摆出"努力查资料"的样子
            # （用哪个表情由 config.LV3_ACTING_EXPRESSION 决定，逻辑在后端）
            self._avatar_emotion(thinking=False, acting=True, speaking=False,
                                 tools=[t.get("tool") for t in result.tools])
        ...
        if not api_resp.degraded and SETTINGS.revoice_with_local and self.local.available:
            ...
            if facts:
                # 表现层：查询结束，换成"她开口说话"
                self._avatar_emotion(thinking=False, acting=False, speaking=True,
                                     tools=[t.get("tool") for t in result.tools])
...
可用命令（在对话中直接输入）：
  ...
  /tools             查看已注册的工具
  /avatar            查看 Live2D 表现层地址与连接状态（Lv3）
    ...
    while True:
        ...
        if raw.startswith("/"):
            ...
            if cmd == "help":
                ...
                # 忘了地址 / 不确定浏览器连上没有，用这个查
                if orch.avatar is None:
                    print(f"  Live2D 表现层未启用。")
                    if orch.avatar_error:
                        print(f"  原因: {orch.avatar_error}")
                    print(f"  启动方式: python main.py --avatar  （或双击 start_live2d.bat）")
                else:
                    st = orch.avatar.state
                    print(f"  表现层地址: {orch.avatar.url}")
                    print(f"  已连接浏览器: {orch.avatar.client_count} 个")
                    print(f"  当前心情: {st.get('mood')} | 主表情: {st.get('primary') or '-'} "
                          f"| 叠加: {st.get('layers') or '-'}")
                    print(f"  可用表情: {', '.join(sorted(orch.avatar.expression_params))}")
                    print(f"  浏览器诊断: {orch.avatar.url}api/diag")
                    print(paint("  ⚠ 必须用上面这个地址访问，不要直接双击 index.html", C_WARN))
            elif cmd == "audit":
...
def _open_browser(url: str, enabled: bool) -> None:
    ...
    让 .bat 去"猜时机"是不可靠的：如果 Ollama 需要启动，主程序会先等几十秒，
    而 .bat 的延迟打开会在这之前触发，页面就会先报"连不上服务"。
    所以改由程序自己开 —— 此时端口已经定下来，地址一定正确。
    """
    if not enabled:
        return
    try:
        import webbrowser
        webbrowser.open(url)
        print(paint(f"  已尝试自动打开浏览器：{url}", C_DIM))
    except Exception as exc:
        print(paint(f"  （自动打开浏览器失败：{exc}，请手动访问上面的地址）", C_DIM))
...
def main() -> int:
    ...
    ap.add_argument("--avatar", action="store_true",
                    help="启动 Live2D 表现层（Lv3），会打印浏览器地址")
    ap.add_argument("--avatar-port", type=int, default=None,
                    help=f"表现层端口（默认 {SETTINGS.avatar_port}；0 = 随机空闲端口）")
    ap.add_argument("--no-open", action="store_true",
                    help="不要自动打开浏览器（默认会自动打开）")
    ap.add_argument("--demo-avatar", action="store_true",
                    help="不对话，只循环展示各种心情（验证模型资产用）")
    args = ap.parse_args()
    ...
    # 只做表现层演示：不对话，循环展示各种心情（用来单独验证模型资产）
    if args.demo_avatar:
        if AvatarServer is None:
            print(f"Live2D 表现层不可用：{AVATAR_IMPORT_ERROR}")
            return 1
        from live2d.avatar_server import _demo
        srv = AvatarServer(port=args.avatar_port if args.avatar_port is not None
                           else SETTINGS.avatar_port, verbose=True)
        srv.start()
        if srv.port_note:
            print(paint(f"\n  {srv.port_note}", C_WARN))
        print(paint(f"\n★ 演示模式已启动，用浏览器打开： {srv.url}", C_MIKU))
        _open_browser(srv.url, SETTINGS.avatar_auto_open and not args.no_open)
        try:
            _demo(srv)
        finally:
            srv.stop()
    ...
    orch = Orchestrator(session_id=args.session, verbose=not args.quiet,
                        ...
                        avatar=args.avatar, avatar_port=args.avatar_port)
    if args.avatar:
        if orch.avatar is not None:
            if orch.avatar.port_note:
                print(paint(f"\n  {orch.avatar.port_note}", C_WARN))
            print(paint(f"\n★ Live2D 表现层已启动，用浏览器打开： {orch.avatar.url}", C_MIKU))
            print(paint("  ⚠ 请用这个地址访问，不要直接双击 live2d/web/index.html", C_WARN))
            _open_browser(orch.avatar.url, SETTINGS.avatar_auto_open and not args.no_open)
        else:
            print(paint(f"\n★ Live2D 表现层启动失败：{orch.avatar_error}", C_WARN))
