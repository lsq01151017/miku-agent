...
Text outside a tool call reaches nobody. The people in the terminal see only what `terminal_send` carries: its `text` argument is the whole message, and it is delivered the moment the call runs. To answer, greet, or react, put the words in that call. A message you did not send through it was not said.
