...
Live2D 播放器库与模型文件（`my-agent/live2d/web`、`my-agent/live2d/models/miku`），只读，没有改过它们。
...
│   ├── extensions/      cortico-world-live2d、cortico-provider-ollama
...
- **局限（如实）**：比版本一重（依赖 pnpm / Node / tsx，启动要 3–10 秒）；
...
- 形象页的水印通过模型自带的开关参数关掉（`Param137=1`），模型文件未改动。
