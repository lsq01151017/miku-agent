...
│   ├── extensions/      cortico-world-live2d、cortico-world-work、cortico-provider-ollama、cortico-provider-chat
│   └── deployments/miku/ 这份部署的配置、记忆、运行数据（不进 git）
├── 启动她.bat / 停止她.bat   版本二的一键启停
├── 启动语音.bat / 停止语音.bat miku 语音服务（GPT-SoVITS，端口 9880）的一键启停
├── 启动语音界面.bat          GPT-SoVITS 推理 WebUI（端口 9872，交互试听/调语言用）
├── voice/                语音方案工作目录（GPT-SoVITS 仓库/环境、底模、miku 微调权重、
│                         参考音频与脚本；数 GB，不入库）
...
- **形象层**：`http://127.0.0.1:18795/` —— 独立 World，自己起 HTTP 服务用 SSE 推通道值；
  ...
  每个通道的值），右下角可取景：缩放、左右、上下、直接拖动画面、复位、记住。底部对话框与控制台
  是同一串消息，输入框下一行按钮带：上下文用量读数、附图、历史、模型选择与真权限（每次问/放行/
  关，第 39–43 轮）——权限落 `worlds.work.permission`，管的是她请 DSH 干活的那双手。
- **工作接口**：她有一件 `work_run` 工具（`cortico-world-work` 扩展包）——把要做的事 POST 给
  DSH 侧动态插件挂的路由（43120），那边驱动一个专属代理会话（`miku-work`）跑完一轮并回传结果：
  审批卡、沙箱、审计全套复用 DSH 的。权限三态由形象页的权限按钮控制；「每次问」下越出 DSH 工作
  区的动作会在 DSH 界面弹审批卡。桥是动态插件，DSH 重启后要在那个会话里重新挂（第 43 轮）。
...
- **局限（如实）**：比版本一重（依赖 pnpm / Node / tsx；启动脚本自己的开销在第 24 轮从约 3 秒
  ...
  控制台前端改一行要重建 Web 产物，且重建时不能有 bot 活着；语音 TTS 服务已就绪（第 44 轮），
  形象层接入未做。
...
# 她的语音服务（TTS，端口 9880；形象层接入前可独立试听）
D:\二面\启动语音.bat
D:\二面\停止语音.bat
...
- 语音 TTS 服务已就绪并定档（第 44–45 轮：GPT-SoVITS **V2PP** + miku 微调模型，无窗口常驻，
  `启动语音.bat`，端口 9880，约 2 秒/句；默认参考与配置见 `voice/miku-voice.json`）；
  但形象层还没接：口型仍是按说话时长跑的振荡，对话回复还没走 TTS。
...
- 形象页的水印通过模型自带的开关参数关掉（`Param137=1`），没有为水印动过模型；模型目录唯一的
  改动是第 24 轮把贴图降到 2048²。
- 工作接口的 DSH 侧是动态插件，不随 DSH 进程持久：DSH 重启后桥不在，她的 `work_run` 会收到
  "桥插件没在跑"的回执；在那个 DSH 会话里重新定义并运行即可（见 `修改记录.md` 第 43 轮）。
