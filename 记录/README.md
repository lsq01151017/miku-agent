...
- 形象页的水印通过模型自带的开关参数关掉（`Param137=1`），没有为水印动过模型；模型目录唯一的
  改动是第 24 轮把贴图降到 2048²。
- 扩展的 `page.test.ts` 有一项断言在改动前（HEAD）就是红的,与第 24 轮无关,未修:页面
  的 `tick` 跳过 `MouthOpen` 通道（口型归说话）,`writeFrame` 只遍历 `current` 里有的通道,
  于是不说话时 World 给的 `MouthOpen`（片段让她张嘴）永远写不到模型上。同文件 8 个
  `FakeNode.value` 类型错误已在第 34 轮清零。
