...
├── 启动语音.bat / 停止语音.bat miku 语音合成（GPT-SoVITS，端口 9880）的一键启停
├── 启动转写.bat / 停止转写.bat 语音转写（faster-whisper，端口 9881，「按住说」用）
├── 启动翻译.bat / 停止翻译.bat 语音翻译（中→日，端口 9882，声音走日语用）
...
- **形象层**：`http://127.0.0.1:18795/` —— 独立 World，自己起 HTTP 服务用 SSE 推通道值；
  ...
  是同一串消息，输入框下一行按钮带：附图、按住说（按键发言，第 48 轮）、上下文用量读数、历史、
  模型选择、声音开关（第 50 轮，落 `worlds.live2d.voiceEnabled`，关了不调 TTS）与真权限
  （每次问/放行/关，第 39–43 轮，落 `worlds.work.permission`，管的是她请 DSH 干活的那双手）。
  她的回复自动走本地 TTS 逐句出声、口型跟真实振幅（第 47–48 轮）；字幕显示她写的原文
  （日常中文对话就是中文），一轮的话逐句堆着、她说完全部才淡掉（第 54 轮）；声音一律
  日语——每段先经本地翻译服务翻成日语再合成（第 52 轮），翻不出来回退原文合成。
...
- **局限（如实）**：比版本一重（依赖 pnpm / Node / tsx；启动脚本自己的开销在第 24 轮从约 3 秒
  ...
  控制台前端改一行要重建 Web 产物，且重建时不能有 bot 活着。
...
# 启动她.bat 会把三个语音服务一并拉起（幂等，已在跑就不动；第 55 轮）
D:\二面\启动她.bat
D:\二面\停止她.bat

# 语音服务的单独启停（平时用不到，只想动某一个时用）
# TTS 合成 9880 / 转写 9881 / 中→日翻译 9882
D:\二面\启动语音.bat
D:\二面\停止语音.bat
D:\二面\启动转写.bat
D:\二面\启动翻译.bat
...
- 语音全链已接（第 47–52 轮）：回复自动逐句出声、口型跟真实振幅、按键发言、声音开关；
  字幕显示原文、声音一律日语（本地翻译服务翻一道再合成）。已知限制：翻译一步走云端中转站
  （deepseek-v4.1-flash，思考关不掉），约 4–5 秒/段——首句出声比纯本地方案慢几秒，
  重复的问候语有缓存免调；语音三服务随 `启动她.bat` 一并拉起（第 55 轮，幂等），bot 只认
  配置地址。语音端口偶被 Windows 动态保留段圈占（开机随机，第 53 轮撞过一次 9880）——
  服务的自举会自己等段释放再绑上，启动器等不到端口不等于服务死了，看 `voice/` 下的日志。
  `停止她.bat` 只停 bot，不停语音服务（TTS 会占着显存）；要全停就再用三个停止 bat。
