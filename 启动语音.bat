...
powershell -NoProfile -ExecutionPolicy Bypass -Command "$c = New-Object System.Net.Sockets.TcpClient; $ok = $false; foreach ($i in 1..90) { try { $t = $c.ConnectAsync('127.0.0.1', 9880); if ($t.Wait(1000) -and $c.Connected) { $ok = $true; break } } catch {} }; if ($ok) { Write-Host 'TTS service is UP on http://127.0.0.1:9880' } else { Write-Host 'not up within 90s - it may be waiting out a Windows-reserved port range; it binds by itself once that clears. See voice\tts-server.log' }"
